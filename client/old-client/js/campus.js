...
window.CampusRenderer = (function () {
  ...
  var anim = {
    ...
    stars: Array.from({ length: 8 }, function(_, i) {
      return { x: 20 + i * 72, y: 5 + (i % 3) * 6, phase: i * 0.8, size: 4 + (i % 2) * 2 };
    }),
  ...
  function spawn(cx, cy) {
    var cols = ['#fbbf24','#f97316','#ec4899','#7c3aed','#10b981','#3b82f6'];
    for (var i = 0; i < 30; i++) {
      var a = (i / 30) * Math.PI * 2, sp = 2 + Math.random() * 3.5;
      anim.particles.push({
        x: cx, y: cy,
  ...
  function drawBackground(ctx, W, H, t) {
    ...
    anim.clouds.forEach(function(cl) {
      ...
      [[0,0,20],[20,-7,16],[40,0,18]].forEach(function(d) {
        ctx.beginPath(); ctx.arc(cx+d[0], cy+d[1], d[2], 0, Math.PI*2); ctx.fill();
      });
    });

    var fc = ['#f97316','#ec4899','#7c3aed','#10b981'];
    for (var j = 0; j < 12; j++) {
      var fx = 20 + j * 50, fy = H-15 + Math.sin(t*0.04+j)*2;
  ...
  function drawUnlocked(ctx, idx, W, H) {
    var img = imgs[idx];
    var cs = getCardSize(W, H); var BW = cs.BW; var BH = cs.BH;
    var x = -BW / 2, y = -BH / 2;
  ...
  function drawLocked(ctx, idx, t, W, H) {
    var cs = getCardSize(W||800, H||500); var BW = cs.BW; var BH = cs.BH;
    var x = -BW / 2, y = -BH / 2;
  ...
  function drawProgress(ctx, W, H, stages) {
    var bX = 10, bY = H-22, bW = W-20, bH = 14;
    ...
    for (var i = 1; i <= 5; i++) {
      var dx = bX + (bW/5)*i - bW/10, dy = bY + bH/2;
  ...
  function animLoop() {
    ...
    if (!canvas || !canvas.isConnected) { anim.rafId = null; return; }

    var ctx = canvas.getContext('2d');
    var W = canvas.width, H = canvas.height, t = anim.frame++;
    ...
    // Confetti particles
    anim.particles = anim.particles.filter(function(p) { return p.life > 0; });
  ...
  function draw(canvasEl, completedStages) {
    ...
    if (!anim.rafId) { anim.frame = 0; animLoop(); }
  }
